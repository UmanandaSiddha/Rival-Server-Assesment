...
import { TASK_NOT_FOUND, TaskCommand } from './task.commands';
...
export class TaskService {
    ...
    async create(user: RequestUser, dto: CreateTaskDto): Promise<any> {
        await this.authorizationService.assertTeamPermission(user.id, dto.teamId, Permission.TASK_CREATE, user.role);
    ...
    async update(user: RequestUser, taskId: string, dto: UpdateTaskDto): Promise<any> {
        ...
        if ('status' in dto) this.authorizationService.assertCan(access, Permission.TASK_UPDATE_STATUS);
        if ('priority' in dto) this.authorizationService.assertCan(access, Permission.TASK_UPDATE_PRIORITY);
        if ('assigneeId' in dto) this.authorizationService.assertCan(access, Permission.TASK_ASSIGN);
    ...
    async remove(user: RequestUser, taskId: string): Promise<{ id: string }> {
        ...
        return this.runCommand({ type: 'delete', taskId, teamId: access.teamId, actorId: user.id });
    }
    ...
    async findOne(user: RequestUser, taskId: string): Promise<any> {
        ...
        const result = await this.databaseService.query(`SELECT * FROM "Task" WHERE "id" = $1 LIMIT 1`, [taskId]);
    ...
    async list(user: RequestUser, query: ListTasksQueryDto) {
        ...
        if (query.teamId) {
            if (!isAdmin) {
                await this.authorizationService.assertTeamMembership(user.id, query.teamId, user.role);
            }
        ...
        const dataSql = `
            ...
            LIMIT $${i++} OFFSET $${i++}
        `;
        const dataResult = await this.databaseService.query(dataSql, [...params, limit, offset]);
