...
import { CreateTaskData, TaskCommand, TASK_NOT_FOUND, UpdateTaskData } from './task.commands';
...
export class TaskProcessor extends WorkerHost {
    ...
    async process(job: Job<TaskCommand>): Promise<any> {
        ...
        switch (command.type) {
            case 'create':
                return this.handleCreate(command.teamId, command.actorId, command.data);
            case 'update':
                return this.handleUpdate(command.teamId, command.taskId, command.actorId, command.data);
            case 'delete':
                return this.handleDelete(command.teamId, command.taskId, command.actorId);
        }
    }

    private async handleCreate(teamId: string, actorId: string, data: CreateTaskData): Promise<any> {
        const task = await this.databaseService.withTransaction(async (client) => {
            const completedAt = data.status === TaskStatus.DONE ? 'now()' : 'NULL';
            ...
            await this.appendActivity(client, row.id, actorId, TaskActivityAction.CREATED, {});
            return toCamelCaseDeep(row);
        });

        await this.publisher.emitToTeam(teamId, 'task.created', { task }, actorId);
        return task;
    }
    ...
    private async handleUpdate(
        ...
        const outcome = await this.databaseService.withTransaction(async (client) => {
            const currentResult = await client.query(`SELECT * FROM "Task" WHERE "id" = $1`, [taskId]);
            ...
            if ('status' in data) {
                ...
                } else if (data.status !== TaskStatus.DONE && current.status === TaskStatus.DONE) {
            ...
            await this.appendActivity(client, taskId, actorId, action, changes);
        ...
        await this.publisher.emitToTeam(
            teamId,
            'task.updated',
            { task: outcome.task, changes: outcome.changes, action: outcome.action },
            actorId,
        );
        return outcome.task;
    }

    private async handleDelete(teamId: string, taskId: string, actorId: string): Promise<{ id: string }> {
        ...
        await this.publisher.emitToTeam(teamId, 'task.deleted', { taskId }, actorId);
        return { id: taskId };
    }
    ...
    private diff(current: any, data: UpdateTaskData): Record<string, { from: any; to: any }> {
    ...
    private deriveAction(current: any, data: UpdateTaskData, changes: Record<string, unknown>): TaskActivityAction {
        if ('status' in changes) {
            if (data.status === TaskStatus.DONE) return TaskActivityAction.COMPLETED;
            if (current.status === TaskStatus.DONE) return TaskActivityAction.REOPENED;
