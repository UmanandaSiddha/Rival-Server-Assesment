...
export class AllExceptionsFilter extends BaseExceptionFilter {
    ...
    private isPgError(exception: unknown): exception is PgErrorLike {
        ...
        if (exception instanceof HttpException) {
            ...
            switch (exception.code) {
                case '23505':
                    ...
                    message =
                        exception.detail ||
                        `Unique constraint failed${exception.constraint ? ` (${exception.constraint})` : ''}`;
                    break;
                case '23503':
                    status = HttpStatus.CONFLICT;
                    message =
                        exception.detail ||
                        `Foreign key constraint failed${exception.constraint ? ` (${exception.constraint})` : ''}`;
                    break;
                case '23502':
        ...
        this.logger.error(
            `Status ${status} - ${rawError} - ${request.url}`,
